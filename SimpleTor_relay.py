...
class RelayState:
    ...
    def link_circuits(self, in_sock, in_circ_id, out_sock, out_circ_id):
        ...
        self.circuit_table[(out_sock, out_circ_id)] = {
            ...
            "crypto_lock": self.circuit_table[(in_sock, in_circ_id)]["crypto_lock"]
        }

relay_state = RelayState()

def handle_server_to_client(server_sock, client_conn, circID, streamID, route):
    try:
        while True:
            app_data = server_sock.recv(stc.RELAY_CELL_DATA_LEN)
            if not app_data: break
            
            padding = b'\x00' * (stc.RELAY_CELL_DATA_LEN - len(app_data))
            padded_data = app_data + padding
            
            with route["crypto_lock"]:
                temp_cell = struct.pack('>BHH4sH498s', stc.RelayCmd.DATA, 0, streamID, b'\x00\x00\x00\x00', len(app_data), padded_data)
                route["bwd_digest"].update(temp_cell)
        ...
        while True:
            ...
            if cellCmd == stc.CellCmd.CREATE:
                try:
                    ...
                    fwd_digest.update(fwd_d_key); bwd_digest.update(bwd_d_key)
                        ...
                        if hmac.compare_digest(expected_digest, received_digest):
                            ...
                            if relayCmd == stc.RelayCmd.BEGIN:
                                ...
                                try:
                                    ...
                                    route["streams"][streamID] = server_sock
                                    
                                    with route["crypto_lock"]:
                                        padding = b'\x00' * stc.RELAY_CELL_DATA_LEN
                                        temp_conn = struct.pack('>BHH4sH498s', stc.RelayCmd.CONNECTED, 0, streamID, b'\x00\x00\x00\x00', 0, padding)
                                        route["bwd_digest"].update(temp_conn)
                                        calc_digest = route["bwd_digest"].copy().finalize()[:4]
                                    ...
                                    print(f"Exit Node failed to connect: {e}")
                                    
                            elif relayCmd == stc.RelayCmd.DATA:
                                actual_message = data[:data_len]
                                if streamID in route.get("streams", {}):
                                    route["streams"][streamID].sendall(actual_message)
